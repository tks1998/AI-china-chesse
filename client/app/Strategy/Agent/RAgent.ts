...
export class RAgent {
    ...
    constructor(team: number, myPieces = null, pastMoves = [], strategy = 0) {
        this.team = team;
        if (myPieces == null)
            this.myPieces = (team == 1 ? RInitGame.RgetRedPieces() : RInitGame.RgetBlackPieces());
        else {
    ...
    computeLegalMoves() {
        this.legalMoves = Rrule.allPossibleMoves(this.myPieces, this.boardState, this.team);
    }
    ...
    movePieceTo(piece: RPiece, pos, isCapture = undefined) {
        piece.moveTo(pos);
    ...
    copy() {
        return new RAgent(this.team, this.myPieces.map(x => x.copy()), this.copyMoves());
    }
